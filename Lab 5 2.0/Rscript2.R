...
loopfunction <- function(n){
  vectorOfMeans <- 1:n
  for(index in 1:n){
    system("java -cp bin sorter.LabCollectionSort data2.txt utfil.txt 600")
...
vectorOfMeansCollection <- loopfunction(100)
vectorOfMeansEgen <- loopfunction2(100)

meanEgen <- mean(vectorOfMeansEgen)
meanCollection <- mean(vectorOfMeansCollection)
